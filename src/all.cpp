...
/*OSil functions. The following 3 functions are for OSil: sil_lab(), sil_lab_swap(), clustyanlys()*/

// [[Rcpp::export]]
void sil_lab(int K, int n, IntegerVector  clus_lab,  IntegerVector  clus_size,  NumericVector disty,  NumericVector dys_j,  NumericVector avg_dys_clus,  NumericVector sil){  /*calculate silhouette's without medoids*/
    int i, j;
    double temp1,temp2;
    for(j = 0; j < n; j++){
        for(i = 0; i < K; i++){
...
void sil_lab_swap(int K, int n, IntegerVector  clus_lab, IntegerVector  alt_clus_lab, IntegerVector  clus_size,  NumericVector disty, IntegerVector  iter,  NumericVector dys_i,  NumericVector avg_dys_clus,  NumericVector silh,  NumericVector altsilh){
    ...
    while(keep_swapping == 1){
        ...
        for(i = 0; i < n; i++){
            for(j = 0; j < K; j++){
                if(alt_clus_lab[i] == j) continue; /* skipping an object already present in a cluster*/
                alt_clus_lab[i] = j;
...
void hosil_lab_swap(int n, NumericVector disty, IntegerVector clus_lab, IntegerVector clus_size,  NumericVector dys_j, NumericVector avg_dys_clus, NumericVector sil, IntegerVector best_clus_lab, IntegerVector all_best_clus_lab, NumericVector all_best_avg_silh, IntegerVector copy_best_clus_lab){
    ...
    for(i = 1; i < n_disty; i++){ /*ignoring first element which is always zero, purposely added. Note it will not effect any other distance which is zero naturally.*/
        if(disty[i] < smdist){
            smdist = disty[i];
            smloc = i;
        }
    ...
    avg_silh = avg_silh/n;  /*here we don't have to compare it with best. As we combine two nearest and it the best we can get*/
    
    all_best_avg_silh[0] =  avg_silh;
    for(int gg = 0; gg<n; gg++){
        all_best_clus_lab[gg] = clus_lab[gg];
    }
    ...
    for(l = 0; l<n-3; l++){ /*generating labels for all hierarchies*/
        ...
        for(i = 0; i < k+1; i++) /*Cluster 1 to combine*/
        {
            for(j = i+1; j < k; j++){ /*Cluster 2 to combine*/
                ...
                /*producing complete set of labels for each pair of cluster*/
                for(int g = 0; g < n; g++){
        ...
        for(int gg = 0; gg<n; gg++){
            all_best_clus_lab[gg+r*n] = best_clus_lab[gg];
        }
        r+=1;
        k = n-r;
    } /* end of l */
